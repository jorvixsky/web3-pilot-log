...
export default function ValidateEntryPopup({
  isLoading,
  data,
  onValidateEntry,
  onClosePopup,
  ...
  return (
    <div
      className={
        "fixed overflow-auto h-full w-full absolute t-0 l-0 size-max z-50 inset-0 " +
    ...
    >
      ...
      <div
        className={
          "absolute w-[42rem] max-h-full over h-fit bg-white m-auto left-0 right-0 top-0 bottom-0 p-12 \
                            border rounded-xl"
        }
      >
        <h1 className="text-4xl font-extrabold dark:text-white text-center mb-24">
          Signing flight details
        </h1>
        ...
        <div id="tof" className={blockSeparationClass}>
          ...
          <p>
            {getTime(
              data?.totalTimeOfFlight?.hours,
              data?.totalTimeOfFlight.minutes
            )}
          </p>
        </div>
        ...
        {showValidateButton && (
          <div
            id="validate"
            className="size-full flex items-center flex-row place-content-center"
          >
